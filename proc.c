...
scheduler(void)
{
  ...
  for(;;){
    ...
    int ticketcounter = 0, totalticket = 0;

    // Calculate the total: 
    // **only** take RUNNABLE proc into consideration
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      if (p->state == RUNNABLE) {
    ...
    int lottery = rand() % totalticket;
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      if (p->state != RUNNABLE)
        continue;
      if (ticketcounter + p->tickets > lottery) {
        break;
      } else {
